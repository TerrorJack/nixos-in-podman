...
To ensure guest programs are only run in an unprivileged virtualized
kernel instance, [`libkrun`](https://github.com/containers/libkrun) is
my personal favorite, since it boots near instantly and works out of
the box with existing podman based workflows. `nixos-in-podman`
supports it in two ways:

- Apply the podman [patch](./patches/podman-krun-init-pid1.patch) for
  the host, and use `podman --runtime=krun` to boot the container
- Enable `--device /dev/kvm` and use `podman --runtime=krun` within
  the container
